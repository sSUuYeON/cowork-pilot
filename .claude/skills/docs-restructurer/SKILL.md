...
4. **Chunk 형식** — 반드시 정확히 이 형식을 따른다. Session Prompt 코드 블록은 **반드시 plain triple-backtick(```` ``` ````)으로만** 열어야 한다. ```` ```markdown ````, ```` ```bash ```` 등 언어 식별자를 붙이면 파서가 깨진다:

   올바른 예시 (4중 backtick으로 감싸서 표현):
   ````
   ## Chunk 1: 프로젝트 초기화
   ...
   잘못된 예시 (파서가 깨지는 패턴들):
   - ```` ```markdown ```` 으로 여는 경우 → 파서가 언어 식별자 때문에 코드 블록을 인식 못함 → **빈 Session Prompt 에러**
   - Session Prompt 안에 ```` ```bash ````, ```` ```json ```` 등 중첩 코드 블록이 있는 경우 → 파서가 첫 번째 ```` ``` ````를 닫는 backtick으로 오인 → **잘린 Session Prompt**
   - Session Prompt 안에 `## Chunk N:` 헤더가 있는 경우 → 파서가 새 Chunk 경계로 오인
...
7. **Session Prompt 파서 호환 규칙** (이 규칙을 어기면 harness가 ESCALATE 에러를 낸다):

   harness 파서는 `### Session Prompt` 아래의 첫 번째 코드 블록(triple-backtick 쌍)에서 프롬프트 텍스트를 추출한다. 파서의 동작 방식을 이해하면 왜 아래 규칙이 필수인지 알 수 있다: 파서는 여는 ```` ``` ````를 만나면 다음 ```` ``` ````까지를 Session Prompt 내용으로 취급한다. 따라서:

   - **언어 식별자 금지**: 여는 코드 블록에 ```` ```markdown ````, ```` ```bash ```` 등 언어 식별자를 붙이지 않는다. 반드시 ```` ``` ```` (plain, 아무것도 뒤에 없음)로만 연다. 파서가 ```` ``` ```` 뒤에 텍스트가 있으면 코드 블록 시작으로 인식하지 못해 빈 프롬프트로 처리한다.
   - **중첩 코드 블록 금지**: Session Prompt 안에 triple-backtick 코드 블록(```` ```bash ````, ```` ```json ````, ```` ```typescript ```` 등)을 절대 넣지 않는다. 파서가 내부의 ```` ``` ````를 닫는 backtick으로 오인하여 프롬프트가 잘린다. 코드 예시가 필요하면 인라인 코드(single backtick)나 4칸 들여쓰기로 대체한다.
...
8. **검증**: 생성된 exec-plan이 파서 규격에 맞는지 확인한다. 하나라도 실패하면 자동 수정 후 재검증 (최대 3회):
   - Chunk 헤더: `^## Chunk (\d+): (.+)$`
   - Completion Criteria: `^- \[([ x])\] (.+)$`
   - Session Prompt가 비어있지 않음
   - Session Prompt 여는 코드 블록이 plain ```` ``` ````임 (```` ```markdown ````, ```` ```text ```` 등 언어 식별자가 없음)
