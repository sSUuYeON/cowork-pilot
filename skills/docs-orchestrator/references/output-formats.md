...
- **중첩 코드 블록 (치명적)**: Session Prompt는 triple-backtick 쌍으로 감싸지므로, 내부에 또 다른 triple-backtick(bash, json, typescript 등)이 있으면 파서가 깨진다. 코드 예시는 인라인 코드(backtick)나 4칸 들여쓰기로 대체한다.
- **`## Chunk N:` 헤더**: 파서가 새로운 Chunk 경계로 오인한다. Session Prompt 안에 마크다운 H2 헤더를 넣지 않는다.
