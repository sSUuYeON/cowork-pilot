...
### Completion Criteria
- [x] src/cowork_pilot/orchestrator_state.py 파일 존재
- [x] DocsOrchestratorConfig dataclass가 config.py에 존재
- [x] load_docs_orchestrator_config() 함수가 config.py에 존재
- [x] pytest tests/test_orchestrator_state.py 통과
- [x] pytest tests/test_config.py 통과
...
--- Chunk 완료 워크플로 ---
1. 테스트 실행: pytest tests/test_orchestrator_state.py tests/test_config.py
   - 빌드/테스트 툴체인 설치가 필요하면 /vm-install:vm-install 스킬을 먼저 사용하라
2. 코드 리뷰: /engineering:code-review 실행 → 발견된 이슈 수정
3. 체크박스 마킹: /chunk-complete:chunk-complete 실행 → Completion Criteria를 검증하고 [x] 마킹
```
...
### Completion Criteria
- [x] src/cowork_pilot/quality_gate.py 파일 존재
- [x] pytest tests/test_quality_gate.py 통과
- [x] tests/fixtures/sample_analysis_report.md 파일 존재
...
--- Chunk 완료 워크플로 ---
1. 테스트 실행: pytest tests/test_quality_gate.py
   - 빌드/테스트 툴체인 설치가 필요하면 /vm-install:vm-install 스킬을 먼저 사용하라
2. 코드 리뷰: /engineering:code-review 실행 → 발견된 이슈 수정
3. 체크박스 마킹: /chunk-complete:chunk-complete 실행 → Completion Criteria를 검증하고 [x] 마킹
```
...
### Completion Criteria
- [x] src/cowork_pilot/orchestrator_prompts.py 파일 존재
- [x] src/cowork_pilot/orchestrator_templates/ 디렉토리에 13개 .j2 파일 존재
- [x] pytest tests/test_orchestrator_prompts.py 통과
...
--- Chunk 완료 워크플로 ---
1. 테스트 실행: pytest tests/test_orchestrator_prompts.py
   - 빌드/테스트 툴체인 설치가 필요하면 /vm-install:vm-install 스킬을 먼저 사용하라
2. 코드 리뷰: /engineering:code-review 실행 → 발견된 이슈 수정
3. 체크박스 마킹: /chunk-complete:chunk-complete 실행 → Completion Criteria를 검증하고 [x] 마킹
```
...
### Completion Criteria
- [x] src/cowork_pilot/docs_orchestrator.py 파일 존재
- [x] run_docs_orchestrator() 함수가 Phase 0, 1, 1.5를 처리
- [x] pytest tests/test_docs_orchestrator.py 통과 (Phase 0→1→1.5 전이 테스트)
...
--- Chunk 완료 워크플로 ---
1. 테스트 실행: pytest tests/test_docs_orchestrator.py
   - 빌드/테스트 툴체인 설치가 필요하면 /vm-install:vm-install 스킬을 먼저 사용하라
2. 코드 리뷰: /engineering:code-review 실행 → 발견된 이슈 수정
3. 체크박스 마킹: /chunk-complete:chunk-complete 실행 → Completion Criteria를 검증하고 [x] 마킹
```
...
### Completion Criteria
- [x] docs_orchestrator.py에 Phase 2 갭 분석 로직 존재
- [x] docs_orchestrator.py에 Phase 3 문서 생성 로직 존재
- [x] 기능 묶음(bundle) 로직이 동작
- [x] pytest tests/test_docs_orchestrator.py 통과 (Phase 2, 3 테스트 포함)
- [x] tests/fixtures/sample_gap_report.md 파일 존재
...
### Completion Criteria
- [x] docs_orchestrator.py에 Phase 4 품질 검토 로직 존재
- [x] docs_orchestrator.py에 Phase 5 exec-plan 생성 로직 존재
- [x] _determine_next_step()이 Phase 0→1→1.5→2→3→4→5 전체 흐름 처리
- [x] pytest tests/test_docs_orchestrator.py 통과 (전체 Phase 테스트)
...
### Completion Criteria
- [x] cowork-pilot --mode docs-orchestrator가 동작
- [x] --docs-mode auto/manual 인자가 동작
- [x] --manual-override 인자가 동작
- [x] skills/docs-orchestrator/SKILL.md 파일 존재
- [x] skills/docs-orchestrator/references/ 디렉토리에 3개 파일 존재
